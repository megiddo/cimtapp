...
CIMTapp encrypts each user sqlite with a per-user DEK, then wraps that DEK with the application master key (AMK) `CIMT_MASTER_KEY`. A backup that omits the AMK cannot be restored.
...
`GET /api/v1/me/export` (cookie session) returns `application/octet-stream` — the caller’s plaintext sqlite. This is the **exfil unit**: it includes the account snapshot, syringes, compounds, and uses.
