...
This repository currently ships the **framework milestone**: Slim 4 JSON API at `/api/v1`, SvelteKit static SPA in `backend/public/`, Docker, and testing gates. Auth, encrypted per-user SQLite, and dose domain land in later stacked branches.
...
`make up` is equivalent to copying `.env` if missing and running `docker compose up --build`. Bind-mounts `backend/` for PHP edits. SQLite files persist in the `cimtapp-data` volume at `/var/www/cimtapp/data`.
...
| `DATA_DIR` | Directory for `global.sqlite` and `users/*.enc` (Docker: `/var/www/cimtapp/data`) |
...
data/             gitignored sqlite / encrypted user stores
```
